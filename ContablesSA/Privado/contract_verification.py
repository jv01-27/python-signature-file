...
def verify_hash(contract_file, expected_hash):
   # Calculate the hash of the downloaded file.
   calculated_hash = calculate_hash(contract_file)
...
parser = argparse.ArgumentParser(description="Verifica el hash del contrato.")
# Define two command-line arguments:
# -f or --file: Path to the downloaded software file (required).
# --hash: Valor de hash esperado (required).
parser.add_argument("-f", "--file", dest="contract_file", required=True, help="Ruta del contrato")
parser.add_argument("--hash", dest="expected_hash", required=True, help="Valor de hash esperado")
...
if not args.contract_file or not args.expected_hash:
   # Print an error message in red using colorama.
   print(f"{Fore.RED}[-] Por favor especifique el contrato a analizar y su hash")
   # Exit the script.
   sys.exit()

   # Check if the hash of the file is accurate by calling the verify_hash function.
if verify_hash(args.contract_file, args.expected_hash):
   # If the hash is accurate, print a success message in green.
   print(f"{Fore.GREEN}[+] Verificación del hash exitosa. El contrato es auténtico.")
else:
   # If the hash does not match, print an error message in red.
   print(f"{Fore.RED}[-] Verificación del hash fallida. EL contrato no es auténtico.")